...
Transcribe remote debug artifact 是例外：为了排查 ChatGPT remote 和页面行为，monitor 会把 raw CDP request/response 细节写到 `remote-debug/transcribe/<timestamp>/<requestId>/`，main process 也会写 DOM snapshot、recorder probe、app recording 和 upload replacement artifact。这些 artifact 不走 `sanitizeLogDetails()`，会保留 headers、postData、response body、transcript 文本、DOM 候选文本、页面 recorder/upload 摘要、app 侧录音和 replacement decision；普通 app JSONL 只记录 `remoteDebugDir` 或 artifact path 指向对应文件。
...
| Level | 用途 | 典型事件 |
| --- | --- | --- |
| `debug` | 高频、正常、只有排查时才需要的细节 | permission 放行/拒绝、页面导航、overlay ready/load、内部 bridge 注册、DOM 候选文本、network fallback 被 DOM 接管、提示音子进程退出码 |
| `info` | 用户动作或关键状态变化 | app boot、开始/结束/取消听写、stop deferred until pending start is sent、start confirmed、app recorder started/stopped、transcribe request observed、remoteDebugDir、DOM snapshot artifact path、recorder probe artifact path、transcribe network fallback scheduled/finalizing、overlay 状态变化、transcribe started/succeeded、transcript finalized、提示音 spawn 发起 |
| `warn` | app 可以继续运行，但当前功能可能降级或需要重试 | start unconfirmed retry、duplicate start skipped、clear input timeout continue、app recorder start/wait timeout、missing transcribe request timeout、stop skipped、CDP monitor attach/Fetch enable 失败、upload replacement continue 失败、transcribe response 没有文本、提示音子进程非 0 退出 |
...
- `remote-debug/transcribe` 是显式排障 artifact，不适用以上 redaction；它会保留 remote headers、postData、response body、transcript 文本、DOM 候选文本、页面 recorder/upload 摘要、app 侧录音和 replacement decision。
