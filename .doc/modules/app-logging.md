...
Transcribe remote debug artifact 是例外：为了排查 ChatGPT remote 行为，monitor 会把 raw CDP request/response 细节写到 `remote-debug/transcribe/<timestamp>/<requestId>/`。这些 artifact 不走 `sanitizeLogDetails()`，会保留 headers、postData、response body 和 transcript 文本；普通 app JSONL 只记录 `remoteDebugDir` 指向对应目录。
...
| `info` | 用户动作或关键状态变化 | app boot、开始/结束/取消听写、stop deferred until pending start is sent、start confirmed、transcribe request observed、remoteDebugDir、transcribe network fallback scheduled/finalizing、overlay 状态变化、transcribe started/succeeded、transcript finalized、提示音 spawn 发起 |
| `warn` | app 可以继续运行，但当前功能可能降级或需要重试 | start unconfirmed retry、duplicate start skipped、clear input timeout continue、missing transcribe request timeout、stop skipped、CDP monitor attach 失败、transcribe response 没有文本、提示音子进程非 0 退出 |
...
- `remote-debug/transcribe` 是显式排障 artifact，不适用以上 redaction；它会保留 remote headers、postData、response body 和 transcript 文本。
